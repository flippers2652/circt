...
getRelativeFieldId(FieldRef changedFieldRef, FIRRTLBaseType connectionType,
...
void IMConstPropPass::mergeOnlyChangedLatticeValue(Value dest, Value src,
  ...
  if (auto relativeSrc =
          getRelativeFieldId(changedFieldRef, baseType, fieldRefSrc))
    mergeLatticeValue(fieldRefDest.getSubField(*relativeSrc),
                      fieldRefSrc.getSubField(*relativeSrc));

  if (auto relativeDest =
          getRelativeFieldId(changedFieldRef, baseType, fieldRefDest))
    mergeLatticeValue(fieldRefDest.getSubField(*relativeDest),
                      fieldRefSrc.getSubField(*relativeDest));
}
...
void IMConstPropPass::visitConnectLike(FConnectLike connect,
  ...
  if (auto relativeSrc =
    ...
    propagateElementLattice(
        *relativeSrc,
        baseType.getFinalTypeByFieldID(*relativeSrc).cast<FIRRTLBaseType>());
